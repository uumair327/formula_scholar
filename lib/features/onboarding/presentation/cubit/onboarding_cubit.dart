...
class OnboardingCubit extends Cubit<OnboardingState> {
  ...
  Future<void> loadCountries() async {
    ...
    final result = await _getCountries.execute();
    switch (result) {
  ...
  Future<void> selectCountry(Country country) async {
    AppLogger.info('Country selected: ${country.name}', tag: AppLogTags.onboardingCubit);
    emit(state.copyWith(
    ...
    // Try finding states
    final result = await _getStates.execute(country.id);
    switch (result) {
      case Success(:final data):
  ...
  void selectStateRegion(StateRegion stateRegion) {
    AppLogger.info('State selected: ${stateRegion.name}', tag: AppLogTags.onboardingCubit);
  ...
  Future<void> confirmLocation() async {
    ...
    final result = await _getBoards.execute(countryId, stateId: stateId);
    switch (result) {
      case Success(:final data):
        emit(state.copyWith(status: OnboardingStatus.loaded, boards: data.data));
      case Error(:final failure):
        emit(state.copyWith(
          status: OnboardingStatus.error,
          errorMessage: failure.message,
  ...
  Future<void> selectBoard(Board board) async {
    AppLogger.info('Board selected: ${board.name}', tag: AppLogTags.onboardingCubit);
    emit(state.copyWith(
      selectedBoard: board,
      selectedGrade: null,
    ...
    final result = await _getGrades.execute(board.id);
    switch (result) {
      case Success(:final data):
        emit(state.copyWith(status: OnboardingStatus.loaded, grades: data.data));
      case Error(:final failure):
        emit(state.copyWith(
          status: OnboardingStatus.error,
          errorMessage: failure.message,
  ...
  void selectGrade(Grade grade) {
    AppLogger.info('Grade selected: ${grade.label}', tag: AppLogTags.onboardingCubit);
    emit(state.copyWith(selectedGrade: grade));
  }

  /// Navigation helpers
  void goBackToLocationSelection() {
    emit(state.copyWith(step: OnboardingStep.locationSelection, status: OnboardingStatus.loaded));
  }

  void goBackToBoards() {
    emit(state.copyWith(step: OnboardingStep.boardSelection, status: OnboardingStatus.loaded));
  }
}
