...
class _GraphPainter extends CustomPainter {
  ...
  double? _evaluate(String latex, double x, Map<String, double> params) {
    // Simplify LaTeX and evaluate. We can match exact patterns for our seed formulas:
    // 1. y = a*x^2 + b*x + c (Quadratic)
    // 2. y = a*sin(b*x + c) (Trig)
    // 3. y = a*(x-h)^2 + k (Vertex)
    // 4. y = m*x + c (Linear)
    // 5. Exponential: y = a * b^x
    
    final lower = latex.toLowerCase().replaceAll(' ', '');
    final double a = params['a'] ?? params['A'] ?? 1.0;
    final double b = params['b'] ?? params['B'] ?? 1.0;
    final double c = params['c'] ?? params['C'] ?? 0.0;
    final double m = params['m'] ?? 1.0;
    final double h = params['h'] ?? 0.0;
    final double k = params['k'] ?? 0.0;

    if (lower.contains('sin')) {
      return a * math.sin(b * x + c);
    }
    if (lower.contains('cos')) {
      return a * math.cos(b * x + c);
    }
    if (lower.contains('^x') || lower.contains('b^x')) {
      final double base = params['b'] ?? 2.0;
      return a * math.pow(base, x).toDouble();
    }
    if (lower.contains('(x-h)')) {
      return a * math.pow(x - h, 2) + k;
    }
    if (lower.contains('x^2')) {
      return a * x * x + b * x + c;
    }
    if (lower.contains('m*x') || lower.contains('mx')) {
      return m * x + c;
    }
    
    // Default polynomial fallback if expression matches simple formulas
    return a * x * x + b * x + c;
  }
